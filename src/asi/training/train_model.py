...
def run_sweep_random_forest(
    ...
    def evaluate_random_forest():
        ...
        random_forest_model = build_random_forest(
            X_train, y_train, n_estimators=wandb.config["n_estimators"], max_depth=wandb.config["max_depth"]
        )

        # Ocena modelu RandomForestClassifier
        evaluate_model(random_forest_model, X_test, y_test, evaluation_metrics_path)
        pickle.dump(random_forest_model, open(model_path["random_forest_model_path"], "wb"))
        wandb_config_dict = {"n_estimators":wandb.config["n_estimators"], "max_depth":wandb.config["max_depth"]}
        experiment_artifact = wandb.Artifact(
            "random_forest_model", type='model',
        ...
        wandb.run.link_artifact(experiment_artifact, "asi-project-2023/my-awesome-project/RandomForestModel")
    ...
    sweep_id = wandb.sweep(project="my-awesome-project", entity="asi-project-2023", sweep=sweep_config)
...
def train_model(
    ...
    run_sweep_random_forest(X_train, X_test, y_train, y_test, evaluation_metrics_path, model_path)
    wandb.init(project="my-awesome-project", entity="asi-project-2023", tags=["LogisticModel"])
    ...
    wandb.init(project="my-awesome-project", entity="asi-project-2023", tags=["DecisionTreeModel"])
    ...
    # artefakt decision tree
    wandb_config_dict = {"n_estimators": wandb.config["n_estimators"], "max_depth": wandb.config["max_depth"]}
    experiment_artifact = wandb.Artifact(
        "decision_tree_model", type='model',
        description='Decision Tree Model',
        metadata=wandb_config_dict
    )
    experiment_artifact.add_file(model_path["decision_tree_model_path"])
    wandb.log_artifact(experiment_artifact)
    wandb.run.link_artifact(experiment_artifact, "asi-project-2023/my-awesome-project/DecisionTreeModel")
    # artfetakt logistic model
    wandb_config_dict = {"n_estimators": wandb.config["n_estimators"], "max_depth": wandb.config["max_depth"]}
    experiment_artifact = wandb.Artifact(
        "logistic_model", type='model',
        description='Logistic Regression Model',
        metadata=wandb_config_dict
    )
    experiment_artifact.add_file(model_path["logistic_model_path"])
    wandb.log_artifact(experiment_artifact)
    wandb.run.link_artifact(experiment_artifact, "asi-project-2023/my-awesome-project/LogisticModel")
